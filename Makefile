...
.PHONY: all test clean deploy fund help install snapshot format anvil zktest #.PHONY just tells make that all of these are not folders and just anytime we have any command here (help, all), we pretty much just always put it in .PHONY

DEFAULT_ANVIL_KEY := 0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80 # This is the private key for the account that we're going to use to deploy our contract in anvil network
