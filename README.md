## Purpose
**I currently learning about web 3 on Updraft Cyfrin about Foundry. This is a smart contract that have been modified or adjusted with foundry, sepolia, etc. Smart contract basically just FundMe contract that have been modified to work in environment that we want to use. There's so much folder like script (for running script on foundry), test (for testing our contract), and src (where the contract deployed or created). This all contract have been modified to learning about gas optimization like constant, immutable, different storage and memory variables, etc. I like a ton of foundry tools that help working in their environment (forget coverage, forge test, forge script, forge snapshot, forge init, forge build, forge fmt, anvil, cast send, cast call). We also learn about refactor code.**
...
```
or you can go through Makefile, go to your terminal and do this command
```shell
make install
```
